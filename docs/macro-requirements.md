...
### 4.4 ILOS(オブジェクトシステム)— 実用サブセット

ISLISP のオブジェクトシステム(ILOS)を実用範囲で実装する(M6 で追加)。

- **クラス定義**: `defclass`(**多重継承**対応)。スロットオプション
  `:initform`(初期値式)`:initarg`(初期化引数名)`:accessor`(読み書き)
  `:reader`(読み取り専用)
- **インスタンス生成**: `(create (class 名前) :slot 値 ...)`
- **総称関数・メソッド**: `defgeneric` / `defmethod`。クラスによる単一
  ディスパッチ(第 1 引数の型で選択)
- **メソッド結合**: プライマリ + `:before` / `:after` / `:around`、
  `call-next-method` / `next-method-p`
- **クラス優先順位**: 多重継承の解決は C3 線形化に準じる(単調性を満たす)
- **組み込み述語**: `instancep` `class-of` `subclassp` `generic-function-p`
- スロット未束縛アクセスや該当メソッドなしはエラーにする

### 4.5 v1 でも実装しないもの

- 完全なコンディションシステム(簡易版のみ)
- ILOS のうち `:metaclass` 指定、スロットの `:boundp`、メソッドの
  修飾子の完全な組み合わせなど高度な機能
